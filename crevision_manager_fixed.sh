...
# CrediVision Manager - Script Unificado CORRIGIDO
...
install_from_zero() {
    ...
    echo ""
    echo -e "${YELLOW}ATENÇÃO: Isso irá instalar o CrediVision do zero${NC}"
...
Type=oneshot
User=$SERVICE_USER
ExecStart=$PROJECT_DIR/crevision_manager_fixed.sh backup-silent
...
remove_system() {
    ...
    echo ""
    echo -e "${RED}ATENÇÃO: Isso irá remover completamente o CrediVision${NC}"
    echo "Esta ação irá:"
