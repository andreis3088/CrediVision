...
# CrediVision Management Script - Com Atualização Automática
...
# Help function
show_help() {
    echo "CrediVision Management Script - Com Atualização Automática"
    echo ""
    echo "Usage: sudo bash manage_with_auto_update.sh [COMMAND]"
    echo ""
    echo "Commands:"
    ...
    echo "  logs-auto       Show auto-update logs"
    ...
    echo "  auto-start      Start auto-update service"
    echo "  auto-stop       Stop auto-update service"
    echo "  auto-status     Show auto-update status"
    echo "  auto-test       Test auto-update system"
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        log_error "This command requires sudo. Use: sudo bash manage_with_auto_update.sh $1"
        exit 1
    fi
}
...
start_services() {
    ...
    # Start auto-update
    log_info "Starting auto-update service..."
...
stop_services() {
    ...
    pkill -f auto_update_kiosk.py 2>/dev/null || true
...
show_status() {
    ...
    systemctl status credivision-auto-update.service --no-pager -l | head -10
    echo ""
    ...
    if curl -s http://localhost:5000/api/config > /dev/null; then
        ...
        echo "Active tabs: $(echo "$response" | python3 -c "
import json, sys
...
" 2>/dev/null || echo "unknown")"
    ...
    if pgrep -f auto_update_kiosk.py > /dev/null; then
        echo "✓ Auto-update process is running"
        echo "Recent logs:"
        tail -5 /tmp/credivision_auto_update.log 2>/dev/null || echo "No log file"
    else
        echo "✗ Auto-update process is not running"
    fi
    echo ""
...
show_auto_update_logs() {
    echo "Auto-Update Logs (last 50 lines):"
    echo "=================================="
    journalctl -u credivision-auto-update.service -n 50 --no-pager
    echo ""
    echo "Application Log:"
    tail -20 /tmp/credivision_auto_update.log 2>/dev/null || echo "No application log"
    echo ""
}
...
start_auto_update() {
    ...
    if systemctl is-active --quiet credivision-auto-update.service; then
        log_info "✓ Auto-update service started"
    else
        log_error "✗ Failed to start auto-update service"
        return 1
    fi
}
...
# Stop auto-update
stop_auto_update() {
    log_info "Stopping auto-update service..."
    systemctl stop credivision-auto-update.service
    pkill -f auto_update_kiosk.py 2>/dev/null || true
    log_info "✓ Auto-update service stopped"
}
...
show_auto_update_status() {
    ...
    echo "=========================="
    systemctl status credivision-auto-update.service --no-pager -l
    echo ""
    
    if pgrep -f auto_update_kiosk.py > /dev/null; then
        echo "✓ Auto-update process is running"
        echo "PID: $(pgrep -f auto_update_kiosk.py)"
        echo "Recent activity:"
        tail -10 /tmp/credivision_auto_update.log 2>/dev/null || echo "No recent activity"
    else
        echo "✗ Auto-update process is not running"
    fi
    echo ""
...
# Test auto-update
test_auto_update() {
    log_info "Testing auto-update system..."
    ...
    if systemctl is-active --quiet credivision-auto-update.service; then
        log_info "✓ Auto-update service is working"
    else
        log_error "✗ Auto-update service failed"
        journalctl -u credivision-auto-update.service -n 10
        return 1
    fi
    
    echo ""
    echo "To test real updates:"
    echo "1. Add/edit a tab in the web interface"
    echo "2. Watch the kiosk restart automatically"
    echo "3. Check logs: sudo bash $0 logs-auto"
}
...
run_diagnose() {
    ...
    if command -v python3 &> /dev/null; then
        ...
        if python3 -c "import watchdog" 2>/dev/null; then
            echo "✓ watchdog module available"
        else
            echo "✗ watchdog module missing"
        fi
    else
    ...
    if pgrep -f auto_update_kiosk.py > /dev/null; then
        echo "✓ Auto-update process running (PID: $(pgrep -f auto_update_kiosk.py))"
    else
        echo "✗ Auto-update process not running"
    fi
    
    if [ -f "/tmp/credivision_auto_update.log" ]; then
        echo "✓ Log file exists"
        echo "Last log entries:"
        tail -3 /tmp/credivision_auto_update.log
    else
...
test_kiosk() {
    ...
    # Wait for interrupt
    trap 'pkill -f simple_kiosk_enhanced.sh; log_info "Kiosk stopped"' INT
    wait
}
...
start_kiosk() {
    ...
    # Start in fullscreen
    sudo -u "$SERVICE_USER" "$PROJECT_DIR/simple_kiosk_enhanced.sh" fullscreen &
...
stop_kiosk() {
    ...
    pkill -f simple_kiosk.sh 2>/dev/null || true
    pkill -f simple_kiosk_enhanced.sh 2>/dev/null || true
    pkill -f firefox 2>/dev/null || true
